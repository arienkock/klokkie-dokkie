...
import {
  ...
  isStruggling, frontierShare, recordAnswer, pickRound,
} from '../src/mastery.js';
...
describe('promotion', () => {
  it('masters a concept after 5 straight correct frontier answers', () => {
    const { matrix, events } = answerN(createMatrix(), 'analog', 'heel_uur', 'frontier', [true, true, true, true, true]);
  ...
  it('masters with 4 of last 5 correct, ending on a correct answer', () => {
    const { matrix, events } = answerN(createMatrix(), 'zin', 'heel_uur', 'frontier', [false, false, true, true, true]);
  ...
  it('never promotes on a wrong answer even if the window qualifies', () => {
    const { matrix, events } = answerN(createMatrix(), 'analog', 'heel_uur', 'frontier', [true, true, true, true, false]);
    expect(matrix.analog.heel_uur.mastered).toBe(false);
    expect(events).toEqual([]);
  });

  it('resets windows on promotion', () => {
    const { matrix } = answerN(createMatrix(), 'analog', 'heel_uur', 'frontier', [true, true, true, true, true]);
...
describe('frontier share / struggling', () => {
  ...
  it('drops to 0.25 with ≥3 wrong in the last 4 frontier answers', () => {
